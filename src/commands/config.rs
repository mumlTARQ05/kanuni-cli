...
pub async fn execute(action: &ConfigAction) -> Result<()> {
    match action {
        ConfigAction::Show => {
            ...
            println!("  {} {}", "Verbose:".white().bold(), 
                if config.verbose { "true".green() } else { "false".white() });
            
            println!("\n  Config file: {}", Config::get_config_path()?.display().to_string().blue());
        }
        ConfigAction::Set { key, value } => {
            ...
            config.save()?;
            println!("{}  Configuration updated: {} = {}", "✓".green(), key.cyan(), value.yellow());
        }
        ConfigAction::Reset => {
            Config::reset()?;
